function [beta eloo A]= lassopcepram(X, y, method, stop, useGram, Gram, trace)
...
%% Input checking
% Set default values.
global ModelInfo ObjectiveInfo ModelInfoAwal ObjectiveInfoAwal ConstraintInfo PCInfo IDInfo
if nargin < 7
    trace = 0;
end
if nargin < 6
...
[n p] = size(X);
nvars = min(n-1,p); % 
maxk = 80*nvars; % Maximum number of iterations
...
vars = vars + 1;
 ...
 beta(k+1,A) =  (X(:,A)\y)';
   ypr = (X(:,A)* beta(k+1,A)');
   eloo(1) = LOOcalc(X(:,A),ypr,y);
   
mu = X(:,A)*beta(k+1,A); % INIKAH? miu = X*beta
% pause
% return
...
%   beta(k+1,A) = beta(k,A) + gamma*w';
  % LARS OLS HYBRID, recompute beta

   beta(k+1,A) = (X(:,A)\y)';

   ypr = (X(:,A)* beta(k+1,A)');
   eloo(k) = LOOcalc(X(:,A),ypr,y);
